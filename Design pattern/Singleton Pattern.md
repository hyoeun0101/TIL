...
- private 생성자를 가지고 있기 때문에 상속을 할 수 없다. 이는 객체지향의 장점인 상속과 다형성을 이용할 수 없다는 의미다.
- 테스트하기 어렵다. 테스트 시 Mock 오브젝트로 대체하기가 힘들다.
- 서버 환경에서 싱글톤을 완전히 보장할 순 없다. 서버에서 클래스 로더를 어떻게 구성하고 있느냐에 따라 싱글톤 패턴도 오브젝트를 여러 개 만들 수 있다.
