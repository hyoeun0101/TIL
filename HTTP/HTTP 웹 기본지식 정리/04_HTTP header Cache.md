...
1. 첫 번째 요청의 응답 헤더에 ETag를 보낸다.

2. 캐시의 유효시간과 ETag를 브라우저 캐시에 저장한다.
...
3. 캐시가 만료된 후 서버에 재요청할 때 ETag 정보를 함께 보낸다.
...
4. 서버에서 클라이언트가 보낸 `If-Non-Match` 정보를 통해 데이터의 변경을 확인한다.
...
- 이는 원 서버에 직접 접근하는 방식이다. 여기서 프록시 캐시를 도입해보자.
