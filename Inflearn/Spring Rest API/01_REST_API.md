...
self-descriptive message, HATEOAS가 지켜지지 않고 있어서 시스템의 독립적인 진화가 불가능하며, 버저닝을 하는 일이 발생.

self-descriptive message : 메세지는 자신을 설명할 수 있어야 한다. 
...
HATEOAS : 응답에 애플리케이션의 상태 변화가 가능한 하이퍼미디어 정보가 들어있어야 한다. 그 하이퍼미디어를 통해 다음 상태로 전이한다. 