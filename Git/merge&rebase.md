...
메인 브랜치와 다른 브랜치 합치기. `메인브랜치에서 다른 브랜치를 merge`하면 메인브랜치에 하나의 커밋이 생성된다. 히스토리에 브랜치들이 남아있는다.
메인 브랜치에서 `git merge new-branch-name`

### 🍎 merge 시 충돌 해결

메인 브랜치와 다른 브랜치에서 파일 내용이 서로 다를 때 충돌이 발생한다.  
...
메인 브랜치의 마지막 커밋에서 브랜치가 뻗어나갔을 때 메인 브랜치에서 merge를 하게 되면 `Fast forward`를 하게 된다.  
...
: 기존의 merge 방식처럼 메인 브랜치에서 새로운 커밋을 생성하려면 `git merge --no --ff 병합할 브랜치명`명령어를 사용해야한다.

2. 3-way-merge
   ![r](https://user-images.githubusercontent.com/96059261/210203371-890d2e71-4d5e-4aa3-a456-42a40c7b7b2b.png)  
   우리가 아는 merge 방식  
   처음 브랜치로 뻗어간 메인브랜치의 커밋과 그 이후의 커밋, 그리고 다른 브랜치의 커밋 이렇게 세 개의 상태를 비교하며 merge해서 3-way-merge라고 한다.
...
여러 커밋이 메인 브랜치로 옮겨지기 때문에 각각 커밋마다 충돌 해결을 해줘야한다.  
충돌 해결 후
