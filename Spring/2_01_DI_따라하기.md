...
AppContext는 Map을 생성하여 @Component 가 붙어있는 클래스를 <클래스 이름 첫글자 소문자, 클래스의 객체> 이렇게 map에 저장한다.   
클래스의 필드에 @Autowired가 붙어있으면, map에서 일치하는 타입을 찾아 필드에 주입한다.   
@Resource가 붙어있으면, map에서 일치하는 이름을 찾아 필드에 주입한다.   
끝!!
