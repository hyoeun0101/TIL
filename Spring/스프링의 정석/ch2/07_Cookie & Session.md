...
- 서버에서 생성 후 브라우저에 저장.
- 최소한의 정보만 사용해야한다. 서버에 전송하지 않고, 웹 브라우저 내에 데이터를 저장하고 싶으면 웹 스토리지(localStorage, sesionStorage)를 사용한다.

**<동작 과정>**

1. 서버로 처음 요청
2. 서버에서 쿠키 생성 후 응답헤더에 쿠키를 추가하여 응답
   - `Set-Cookie : sessionId=QWE; expires=Sat, 26-Dec-2020 00:00:00 GMT; path=/;domain=.google.com;Secure`
3. 브라우저에 쿠키 저장
4. 다음 요청 시 쿠키를 요청 헤더에 넣어서 요청함.
   - `Cookie: sessionId=QWE`
   - 요청할 때 요청할 서버와 동일한 domain, path을 가진 쿠키를 요청 헤더에 자동으로 넣는다.
