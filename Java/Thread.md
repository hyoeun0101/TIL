## 🍎 프로세스 vs 쓰레드
- 프로세스?
    - 메모리를 할당받아 실제 실행 중인 프로그램을 말한다. 쓰레드 + 자원
- 쓰레드?
    - 프로세스의 자원을 이용해서 작업하는 것을 말한다.
- 프로세스는 공장, 쓰레드는 일꾼이라 생각하면 된다.
- 프로세스에는 반드시 하나 이상의 쓰레드가 존재한다.
- 하나의 프로세스에서 여러 개의 쓰레드가 작업하는 것을 `멀티 쓰레딩`이라 한다.
- `멀티 태스킹`? = 다중 작업. 여러 프로세스가 동시에 작업하는 것.
- 하나의 프로세스가 가질 수 있는 쓰레드 개수는 제한이 없다. 쓰레드가 작업하는데 필요한 메모리 크기때문에 제한이 있는 것이다. 하지만 실제로 메모리 한계에 다다를 정도로 쓰레드를 생성하는 일은 없다.

## 🍎 쓰레드
- 싱글 쓰레드
    - 쓰레드가 하나만 존재할 수 있으며, 한 번에 한 가지 작업만 할 수 있다.
- 멀티 쓰레드
    - 동시에 여러 작업을 할 수 있다. 우리가 채팅을 하는 동시에 파일을 다운로드를 받을 수 있는 것은 멀티 쓰레드로 작성되어 있기 때문이다.
...
### 멀티 쓰레드의 단점
- 동기화와 데드락
    - 데드락(쿄착 상태) : 두 쓰레드가 자원을 점유한 상태에서 서로의 자원을 점유하려고 대기하느라 실행을 멈춘 상태
- 항상 멀티쓰레드가 빠른 것은 아니다. 멀티쓰레드는 작업 전환 시(context switching) 현재 진행 중이던 작업 상태를 저장하고 읽어오는 시간이 소요된다. 때문에 간단한 계산같은 경우 싱글쓰레드가 더 빠르다.


### 싱글 쓰레드의 문제점

- I/O 블락킹 발생
    - 파일 다운로드 작업과 메세지 전송 작업을 동시에 실행한다고 해보자. 싱글 쓰레드는 사용자가 메세지 입력을 마칠 때까지 파일을 다운로드 받지 못하고 계속 대기하게 된다. 이러한 현상을 `I/O 블락킹`이라고 한다. 멀티 쓰레드라면 두 작업을 동시에 할 수 있기 때문에 사용자가 메세지를 입력하는 동안 파일을 다운받을 수 있다.
...
1. 메인 쓰레드에서 start()를 호출해야 쓰레드가 실행된다.
...
- 쓰레드에 우선순위를 부여하여 중요한 작업을 먼저 실행하도록 한다. 우선순위가 높을 수록 쓰레드의 작업 시간이 더 길다.
- main 쓰레드의 우선순위는 5이며, main 메서드 내에서 생성되는 쓰레드의 우선순위도 역시 5이다.
- 쓰레드를 실행하기 전에만 우선순위를 부여할 수 있다. 즉 start() 호출 전에만 setPriority() 호출 할 수 있다.
- 하지만!! 멀티 코어에서 우선순위를 주는 건 의미가 없을 수도 있다. 중요한 건 OS의 `스케줄러 방식`이다. 
...
## 🍎 쓰레드의 동기화
- 같은 프로세스 내에 있는 여러 쓰레드가 프로세스 내의 같은 자원을 공유하면 서로의 작업에 영향을 미친다.
- 하나의 쓰레드가 작업하는 동안 다른 쓰레드는 접근 하지 못하게 막는 것을 `쓰레드 동기화`라고 한다.
- 동기화 해결 : 임계영역(critical section)과 Lock
    - 공유 데이터를 사용하는 코드 영역을 임계영역으로 지정한다. 공유 데이터(객체)는 lock을 가지고 있으며 이 lock을 획득한 쓰레드만이 그 공유 데이터(객체)에 접근할 수 있다.

### 1. synchronized

1. synchronized 메서드 : 메서드 전체를 임계영역으로 지정하기
    - 쓰레드는 synchronized 메서드가 호출되면 해당 메서드가 포함된 객체의 lock을 얻어 작업을 수행하고, synchronized 메서드가 종료되면 lock을 반환한다.
```java
public synchronized void methodName(){}
```


2. synchronized 블럭 : 특정한 영역을 임계영역으로 지정하기
...
- 주의할 점은 공유 데이터인 balance가 private이라는 것이다. private으로 외부로부터의 접근을 막아야 동기화
