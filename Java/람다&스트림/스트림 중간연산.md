...
Stream<Dish> fishMenu = specialMenu.stream().filter(menu -> menu.getType == Dish.Type.FISH); //생선 요리만 필터링
```
...
// 단순 실행. 복호화할 때 사용함!
couponList.stream().peek(CouponGift::setDecrypt).collect(Collectors.toList());
```
