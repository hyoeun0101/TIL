...
1. `상위 클래스 변경의 어려움` : 클래스를 상속받는다는 것은 클래스에 의존한다는 뜻이다. 상위 클래스를 변경하면 하위 클래스에 영향을 미친다.
2. `클래스의 불필요한 증가` : 유사한 기능을 확장하는데 있어서 클래스가 불필요하게 증가될 수 있다.
3. `상속의 오용` : 잘못된 관계로 상속을 하게 되면 필요하지 않은 상위 클래스의 메서드를 사용할 수도 있다.

- 따라서 상속관계는 IS-A(같은 책임을 가질 때) 일 때 사용하고, 기능을 재사용할 경우 다른 객체를 필드로 사용하는 포함 관계를 사용하는 것이 좋다.

### 포함 관계와 상속 관계? 어떻게 구분지을까?

- 포함 관계 : Have A. 원은 점을 가지고 있다.
- 상속 관계 : IS A.스포츠카는 자동차이다.
- 다음을 포함 관계라고 한다.
