...
- static 내부 클래스인 Builder 클래스가 존재.
- 모든 필드를 가지는 private 생성자 필수.
...
public class NutritionFacts {
    ...
    private NutritionFacts(Builder builder) { // Builder 대신 모든 필드를 가지는 생성자도 가능.
...
- NutritionFacts 클래스는 불변이다. 그럼 빌더 패턴으로 객체 생성하면 객체 변경못한다.
