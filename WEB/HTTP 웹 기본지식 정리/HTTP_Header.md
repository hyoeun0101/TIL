...
- 단 Content-Length를 같이 사용할 수 없다. 데이터를 쪼개서 전송하기 때문에 데이터의 전체 크기를 예상할 수 없다. 분할해서 보낼 데이터에 바이트가 명시된다.
...
- 예) Retry-After: Fri 31 Dec 2000 23:59:59 GMT (날짜 표기)
...
- expires, max-age (생명주기)
  - 예) Set-Cookie:expires=Sat, 26-Dec-2020 00:00:00 GMT (만료일이 되면 쿠키 삭제)
  - 예) Set-Cookie:max-age=3600(3600초, 0이나 음수를 지정하면 쿠키 삭제)
- 만료 날짜 생략하면 브라우저 종료 시 삭제
- domain
  - 예) Set-Cookie:domain=example.org
...
- 60초동안 데이터를 캐시에 담아둔다. 이제 같은 요청이 오면 캐시의 유효시간을 검증한 다음, 캐시에서 데이터를 꺼내 사용한다.
- 유효 시간이 지났다면 다시 서버로부터 데이터를 받고, 캐시를 갱신한다.
